...
void find_file(char* dir_name, char* file_to_find)
{
	...
	while((dirp = readdir(dp)) != NULL){		
		...
		if(dirp->d_type == DT_DIR){
			...
			//Construct the next directory: dir_name/dirp->d_name 
			strcpy(new_dir_name, dir_name);
			strcat(new_dir_name, "/");
			strcat(new_dir_name, dirp->d_name);
			//find the file recursively in that directory
			find_file(new_dir_name, file_to_find);
		}
		//if it is not directory, compare the names
		else{
