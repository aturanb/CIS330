              ...
              int* color, int* distance)
{
    ...
    while(!done) {
        ...
        // given a vector of source vetices, find the neighbors by using spmv 
	for(int i = 0 ; i < rows; i++){
	...
	for(int i = 0; i < rows; i++){
		if(dst[i] != 0 && color[i] == 0){
			color[i] = 2;
			distance[i] = iter;
			done = 0; //If there is a newly discovered neighbor, not done yet 
		}
	}

	//Copy dst to src
	for(int i = 0; i < rows; i++){ src[i] = dst[i]; }	
