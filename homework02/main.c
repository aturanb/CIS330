...
int get_num_ints(char** argv)
{
	...
	int ctr_one = 0;
	int ctr_two = 0;
	char buf_one[MAX_NUM_LENGTH];
	char buf_two[MAX_NUM_LENGTH];
	
	//Count the lines for argv[1]
	file = fopen(argv[1], "r");
	if(file == NULL){ exit(EXIT_FAILURE); }
	while((fgets(buf_one, MAX_NUM_LENGTH, file))!= NULL){ ctr_one++; }	
	fclose(file);
	
	//Count the lines for argv[2]
	file = fopen(argv[2], "r");
	if(file == NULL){ exit(EXIT_FAILURE); }
	while((fgets(buf_two, MAX_NUM_LENGTH, file)) != NULL){ ctr_two++; }
        fclose(file);	
	
	//Compare and return respectively
	if(ctr_two == ctr_one){ return ctr_one;	}
	else{ return -1; }
              ...
              unsigned long int* output, int num_ints)
{
	...
	//Copy ints at argv[1] to input_one array
	file = fopen(argv[1], "r");
	if(file == NULL){ exit(EXIT_FAILURE); }
	for(i = 0; i < num_ints; i++){
		fgets(buf_one, MAX_NUM_LENGTH, file);
		input_one[i] = strtol(buf_one, NULL, 10);
	}
	fclose(file);
	
	//Copy ints at argv[2] to input_two array
	file = fopen(argv[2], "r");
	if(file == NULL){ exit(EXIT_FAILURE); }
	for(i = 0; i < num_ints; i++){
		fgets(buf_two, MAX_NUM_LENGTH, file);
		input_two[i] = strtol(buf_two, NULL, 10);
	}
	fclose(file);
...
void sum_ints(unsigned int* input_one, unsigned int* input_two, 
              unsigned long int* output, int num_ints)
{
	//Sum input_one and input_two, cast to unsigned long it, assigned to output
                 ...
                 unsigned long int* output, int num_ints)
{
	...
	// Write to output line by line from output[]
	for(int i = 0; i < num_ints; i++){
		fprintf(file, "%lu\n", output[i]);
	}
	fclose(file);	
